...
// lsh_expr: and_expr
    fir::Expr::Ptr Parser::parseLshExpr() {
        fir::Expr::Ptr expr = parseRshExpr();

        while (tryConsume(Token::Type::LLA)) {
            auto lshExpr = std::make_shared<fir::LshExpr>();

            lshExpr->lhs = expr;
            lshExpr->rhs = parseRshExpr();
    ...
    fir::Expr::Ptr Parser::parseRshExpr() {
        fir::Expr::Ptr expr = parseAndExpr();

        while (tryConsume(Token::Type::RRA)) {
            auto rshExpr = std::make_shared<fir::RshExpr>();

            rshExpr->lhs = expr;
            rshExpr->rhs = parseAndExpr();
        ...
        while (true) {
            ...
            switch (peek().type) {
                ...
                case Token::Type::MINUS:
                    ...
                    break;
             /*   case Token::Type::LLA:
                    consume(Token::Type::LLA);
                    addExpr = std::make_shared<fir::LshExpr>();
                    break;
                case Token::Type::RRA:
                    consume(Token::Type::RRA);
                    addExpr = std::make_shared<fir::RshExpr>();
                    break; */
                default:
                    return expr;
            }
...
// tensor_component_type: 'int' | 'float' | 'bool' | 'complex'
    fir::ScalarType::Ptr Parser::parseTensorComponentType() {
        ...
        switch (peek().type) {
            case Token::Type::PROP_T:
                consume(Token::Type::PROP_T);
                scalarType->type = fir::ScalarType::Type::PROP_T;
                break;
            case Token::Type::INT:
