...
//            std::cout << std::endl;
        ...
        high_level_schedule::ProgramScheduleNode::Ptr
        high_level_schedule::ProgramScheduleNode::configApplyPartition(std::string apply_partition){
            // If no schedule has been constructed, construct a new one
            if (schedule_ == nullptr) {
                schedule_ = new Schedule();
            }
            auto thundergp = ThunderGP_Schedule();
            if (apply_partition == "normal"){
                //thundergp.setThunderGPpartition(ThunderGP_Schedule::Partition::normal);
                (*schedule_->thundergp_schedule).partition = ThunderGP_Schedule::Partition::normal;
            }else if (apply_partition == "secondOrderEstimator"){
                //thundergp.setThunderGPpartition(ThunderGP_Schedule::Partition::secondOrderEstimator);
                (*schedule_->thundergp_schedule).partition = ThunderGP_Schedule::Partition::secondOrderEstimator;
            }else{
                std::cout << "unsupported partition: " << apply_partition << std::endl;
                throw "Unsupported Schedule!";
            }
           // (*schedule_->thundergp_schedule) = thundergp;
        };
        high_level_schedule::ProgramScheduleNode::Ptr
