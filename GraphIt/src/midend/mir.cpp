...
namespace graphit {
    namespace mir {
        ...
        void LshExpr::copy(MIRNode::Ptr node) {
            const auto expr = mir::to<LshExpr>(node);
            BinaryExpr::copy(expr);
        }

        MIRNode::Ptr LshExpr::cloneNode() {
            const auto node = std::make_shared<LshExpr>();
            node->copy(shared_from_this());
            return node;
        }

        void RshExpr::copy(MIRNode::Ptr node) {
            const auto expr = mir::to<RshExpr>(node);
            BinaryExpr::copy(expr);
        }

        MIRNode::Ptr RshExpr::cloneNode() {
            const auto node = std::make_shared<RshExpr>();
            node->copy(shared_from_this());
            return node;
        }
