...
namespace graphit {
    ...
    void MIREmitter::visit(fir::ScalarType::Ptr type) {
        ...
        switch (type->type) {
            case fir::ScalarType::Type::PROP_T:
                output->type = mir::ScalarType::Type::PROP_T;
                retType = output;
                break;
...
//        }
    ...
    void MIREmitter::visit(fir::LshExpr::Ptr fir_expr) {
        auto mir_expr = std::make_shared<mir::LshExpr>();
        mir_expr->lhs = emitExpr(fir_expr->lhs);
        mir_expr->rhs = emitExpr(fir_expr->rhs);
        retExpr = mir_expr;
    };

    void MIREmitter::visit(fir::RshExpr::Ptr fir_expr) {
        auto mir_expr = std::make_shared<mir::RshExpr>();
        mir_expr->lhs = emitExpr(fir_expr->lhs);
        mir_expr->rhs = emitExpr(fir_expr->rhs);
        retExpr = mir_expr;
    };
