...
#pragma HLS ARRAY_PARTITION variable=filter_num dim=0 complete
        ...
        burstRead(0, edge_end, edgesHeadArray, mapStream);//depth=2
        sliceStream(mapStream, mapStreamTmp);//depth=QUEUE_SIZE_MEMORY
        sliceStream(mapStreamTmp, mapSliceStream);//depth=2
