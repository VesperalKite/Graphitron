...
namespace graphitron {
    ...
    void CodeGen::genKernelRunner() {
        ...
        for (int i = 0; i < vp_funcs.size(); i++) {
            ...
            oss << "        int argvi = 0;" << endl;
            oss << "        int partVertexNum = ((partition->dstEnd - partition->dstStart)/(ALIGN_SIZE) + 1)*(ALIGN_SIZE);" << endl;
    ...
    void CodeGen::genPropertyArrayDecl(mir::VarDecl::Ptr var_decl) {
    ...
    void CodeGen::genScalarDecl(mir::VarDecl::Ptr var_decl) {
        var_decl->type->accept(type_visitor);
        oss << var_decl->name << ";"<<endl;
        AddScalarArg(var_decl);
    }

    void CodeGen::AddUserMem(mir::VarDecl::Ptr var_decl){
    ...
    void CodeGen::AddScalarArg(mir::VarDecl::Ptr var_decl) {
    ...
    void CodeGen::gen_he_mem_config_h(mir::VarDecl::Ptr var_decl) {
        auto var = mir_context_->getSymbol(var_decl->name);
        string mem_name = var.getAlias();
        mir::VectorType::Ptr vector_type = std::dynamic_pointer_cast<mir::VectorType>(var_decl->type);
        auto vector_element_type = vector_type->vector_element_type;
    ...
    void CodeGen::gen_he_mem_id_h(mir::VarDecl::Ptr var_decl) {
        auto var = mir_context_->getSymbol(var_decl->name);
        string mem_name = var.getAlias();
    ...
    void CodeGen::gen_vp_kernel_cpp(mir::VarDecl::Ptr var_decl) {
        TypeGenerator* type_printer = new TypeGenerator(mir_context_, vp_kernel_cpp_buffer1);
        mir::VectorType::Ptr vector_type = std::dynamic_pointer_cast<mir::VectorType>(var_decl->type);
        vp_kernel_cpp_buffer1 << "        ";
    ...
    void CodeGen::gen_ep_kernel_cpp(mir::VarDecl::Ptr var_decl) {
    ...
    void CodeGen::gen_vp_kernel_mk(mir::VarDecl::Ptr var_decl) {
    ...
    void CodeGen::gen_ep_kernel_mk(mir::VarDecl::Ptr var_decl) {
    ...
    void CodeGen::gen_set_kernel(mir::VarDecl::Ptr var_decl) {
        auto var = mir_context_->getSymbol(var_decl->name);
        string mem_name = var.getAlias();
    ...
    void CodeGen::gen_host_graph_partition_cpp(mir::VarDecl::Ptr var_decl) {
        auto var = mir_context_->getSymbol(var_decl->name);
        string mem_name = var.getAlias();
        host_graph_partition_cpp_buffer << "        MEM_ID_" << expr_visitor->toUpper(mem_name) << "," << endl;
    }

    void CodeGen::visit(mir::FuncDecl::Ptr func) {
        if(func->isFunctor) {
            if(func->name == "main") {
                genMainBody();
            } else {
    ...
    void CodeGen::genPropertyArrayAlloc(mir::VarDecl::Ptr var_decl) {
        auto var = mir_context_->getSymbol(var_decl->name);
        string mem_name = var.getAlias();
        printIndent();
        oss << var_decl->name;
    ...
    void CodeGen::genScalarVectorAlloc(mir::VarDecl::Ptr var_decl, mir::VectorType::Ptr vector_type) {
        printIndent();
    ...
    void CodeGen::genScalarAlloc(mir::VarDecl::Ptr var_decl) {
        printIndent();
        oss << var_decl->name << " ";
    ...
    int CodeGen::genNewfiles() {
        util::replaceFile(root_path+"/lib/template/kernel/vp_kernel.cpp",
        ...
        util::insertFile(root_path+"/lib/template/kernel/ep_kernel.cpp",
        ...
        util::insertFile(root_path+"/lib/template/kernel/vp_kernel.mk",
                        output_path_+"/../tmp/vp_kernel.mk",
                        "# insert",
                        vp_kernel_mk_buffer);
        util::insertFile(root_path+"/lib/template/kernel/ep_kernel.mk",
        ...
        util::insertFile(root_path+"/lib/libgraph/host_graph_data_structure.h",
    ...
    void CodeGen::gen_VertexProcess() {
    ...
    void CodeGen::gen_vp_func_kernel_cpp(mir::FuncDecl::Ptr vp_func) {
    ...
    void CodeGen::gen_ep_func_kernel_cpp(mir::FuncDecl::Ptr ep_func) {
        std::stringstream func_body;
        unsigned int indent_level = 6;
        StmtGenerator* stmt_printer = new StmtGenerator(mir_context_, func_body, &indent_level);
